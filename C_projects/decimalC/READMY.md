# Decimal library C

Статическая библиотека для работы с decimal значениями реализованная на языке C.

Version - 1.0.0.

![alt text](Images/image.png)

## 📋 Оглавление

- [Features] (#Features)

## ✨ Features

- ✅ Библиотека разработана на языке С стандарта C11 с использованием компилятора gcc.
- ✅ Ориентир на стандарт POSIX.1-2017. Код отформатирован Google Style.
- ✅ Библиотека разработана в соответствии с принципами структурного программирования.
- ✅ Сборка производится с помощью Makefile.
- Цели Makefile:
▪ all
▪ danya_decimal.a
▪ test
▪ gcov_report
▪ clean
- ✅ Тип данных decimal реализован с использованием целочисленных массивов int bits[4], с соблюдением разрядов числа в нем.
- ✅ В реализации использован тип данных Big_Decimal.
- ✅ В реализации не используется тип данных int128_t.
- ✅ Определяемый поддерживает числа от -79,228,162,514,264,337,593,543,950,335 до +79,228,162,514,264,337,593,543,950,335.

## 📋 Usage

### ----- Preliminary requirements

- Compiler: gcc с поддержкой стандарта C_11 или новее.
- Система сборки: Make.

### ----- installing 

#### Если необходимо клонировать только отдельный проект:
- Создаем репозиторий (при его отсутствии):  
mkdir project-folder && cd project-folder
git init
- Включаем sparse checkout:  
git config core.sparseCheckout true
- Указываем нужную папку с проектом:  
echo "C_projects/decimalC/" >> .git/info/sparse-checkout
- Добавляем удаленный репозиторий:  
git remote add origin git@github.com:InspectorGaishnik/Portfolio.git
- Скачиваем только указанную папку:  
git pull origin main
- Перейти в папку нужного проекта:  
cd C_projects/decimalC/src/
- Запустить Make, для компиляции статической библиотеки:  
make -> danya_decimal.a

#### Если необходимо клонировать весь репозиторий:
- Клонируем репозиторий:  
git clone git@github.com:InspectorGaishnik/Portfolio.git
- Перейти в папку нужного проекта:  
cd Portfolio/C_projects/decimalC/src/
- Запустить Make, для компиляции статической библиотеки:  
make -> danya_decimal.a

### ----- launch

- Для использования необходимо создать main.c, объявить в нем danya_additional.h как заголовочный файл и при компиляции исполняемого файла, подключать библиотеку danya_decimal.a:  
Пример команды компиляции:  
gcc -Wall -Werror -Wextra main.c -o main.out danya_matrix.a 

### ----- Testing

- Перейти в директорию src:  
cd project-folder/C_projects/decimalC/src/
- Компиляция исполняемых файлов и их исполнение:   
make test
